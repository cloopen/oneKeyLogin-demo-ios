...
- (void)removeAllSubviews:(UIView *)targetView
{
    for (UIView *subview in targetView.subviews) [subview removeFromSuperview];
}


-(void)viewWillLayoutSubviews{
    [super viewWillLayoutSubviews];
    self.xyPageScrollView.frame = self.view.bounds;
    self.xyPageControl.frame = [self xyLayoutPageControlFrame];
    self.xyEnterBtn.frame = [self xyLayoutEnterBtnFrame];
    if (self.xyPageScrollView.subviews.count == 0) {
        [self xyReloadPage];
    }
}
...
- (void)xyBringSubViewToFront{
    ...
    [self.view bringSubviewToFront:self.xyPageControl];
...
- (void)xyAddPageScroll{
    ...
    _xyPageControl.pageIndicatorTintColor = [UIColor lightGrayColor];
    _xyPageControl.currentPageIndicatorTintColor = [UIColor grayColor];
    [self.view addSubview:_xyPageControl];
    [self.view addSubview:_xyEnterBtn];
...
- (CGRect)xyLayoutEnterBtnFrame{
    ...
    size = CGSizeMake(MIN(self.view.frame.size.width,self.view.frame.size.height) * 0.6, 40);
    return CGRectMake(self.view.frame.size.width/2 -size.width/2,_xyPageControl.frame.origin.y - size.height, size.width, size.height);
}
...
- (void)xyReloadPage{
    ...
    [_xyPageScrollView setContentOffset:CGPointMake(_currentPage * self.view.bounds.size.width,0) animated:NO];
    ...
    if([self xyIsLastPage:_xyPageControl]){
        ...
        if(_xyPageControl.alpha == 1){
            [UIView animateWithDuration:1 animations:^{
                self->_xyEnterBtn.alpha = 1;
                self->_xyPageControl.alpha = 0;
            }];
        }
    }else{
        if(_xyPageControl.alpha == 0){
            [UIView animateWithDuration:1 animations:^{
                self->_xyEnterBtn.alpha = 0;//akai
                self->_xyPageControl.alpha = 1;
            }];
        }
    }
}
...
//添加通知
-(void)addNotificationForRotate{
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(handleStatusBarOrientationWillChange:)
...
#ifdef DEBUG
//    NSInteger willChangeInterfaceOrientation = [[notification.userInfo objectForKey:@"UIApplicationStatusBarOrientationUserInfoKey"]integerValue];
...
#endif
    [self removeAllSubviews:self.xyPageScrollView];
    _xyPageArr = nil;
    _currentPage = _xyPageControl.currentPage;
}
@end
